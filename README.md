...
### 📌 eeprom.c: Non-Volatile Memory & Wear Leveling (`nvm_driver.c`)
*   **Silicon Lifespan Preservation:** Employs a ring-buffered tracking pointer (`local_index += 4`) to sequentially step continuous 1-second process data logs across a 4MB SPI Nor Flash chip, protecting the silicon from premature single-address burnouts.
*   **Look-Ahead Block Management:** Intercepts upcoming sector boundaries (`index % SECTOR_SIZE == 4096`) to trigger asynchronous sector erases ahead of active write states, completely removing write-latency execution drops.
...
*   
### 📌 Module A: Deterministic Runtime Executive (`main.c`)
...
### 📌 Module B: Real-Time Signal Processing & Precision Math (`totalizer.c`)
...
### 📌 Module E: Asynchronous Input Capture & Speed Tracking (`proximity_sensor.c`)
...
### 📌 Module F: Low-Level I2C Interface & HMI Coordinate Decoding (`touch_driver.c`)
